...
const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ shapes, selectedId, onSelect, onDoubleClick, legendItems = [] }) => {
  ...
  const downloadPNG = () => {
    ...
    // Create a copy to manipulate for export if needed, 
    // but since we are including legend in SVG now, we can just use it.
  ...
  const renderShape = (shape: Shape) => {
    ...
    switch (shape.type) {
      ...
      case 'text':
        ...
        return (
          <g key={shape.id}>
            {/* Halo for readability on dark patterns */}
            {isPercentage && (
               <text 
                x={shape.x} 
                y={shape.y} 
  ...
  return (
    <div className="relative group bg-slate-800 rounded-xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        {/* SVG-based Legend for export inclusion */}
        {hasLegend && (
          <g transform={`translate(50, ${CANVAS_HEIGHT + 20})`}>
             <text x="0" y="5" fontSize="10" fontWeight="bold" fill="#64748b" textAnchor="start">CHÚ THÍCH:</text>
