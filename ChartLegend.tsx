...
const ChartLegend: React.FC<ChartLegendProps> = ({ items, onDoubleClick }) => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-6 p-4 bg-slate-900/50 border border-slate-800 rounded-xl mt-4 animate-in fade-in slide-in-from-bottom-2">
      <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mr-2">Kích đúp nhãn để sửa:</span>
      {items.map((item, i) => (
        <div 
          key={i} 
